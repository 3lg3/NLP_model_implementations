...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        i = -1
        while stack_len > 0 and i >= -3:
            w = state.stack[i]
            # <Root>
            if pos[w] == "CD":
                res.append(self.word_vocab["<CD>"])
            elif pos[w] == "NNP":
                res.append(self.word_vocab["<NNP>"])
            elif words[w] not in self.word_vocab.keys():
                res.append((self.word_vocab["<UNK>"]))
            elif words[w] is None:  # <ROOT>
                res.append(self.word_vocab["<ROOT>"])
            else:  # not special symbol
                res.append(self.word_vocab[words[w].lower()])
            stack_len -= 1
            i -= 1
        while i >= -3:
            # append <NULL> filler
            res.append(self.word_vocab["<NULL>"])
            i -= 1

        # append the buffer
        buffer_len = len(state.buffer)
        i = -1
        while buffer_len > 0 and i >= -3:
            w = state.buffer[i]
            # <Root>
            if pos[w] == "CD":
                res.append(self.word_vocab["<CD>"])
